...
<head>
    ...
    <style>
        ...
        #newMessageMenu input,
        #newMessageMenu textarea {
            width: 100%;
            padding: 5px;
            margin-bottom: 10px;
            background-color: var(--night-bg);
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            messageDropdown.addEventListener('click', function(e) {
                e.preventDefault();
                messageMenu.classList.toggle('dropdown-menu-show');
            });

            // Close the dropdown when clicking outside
            document.addEventListener('click', function(e) {
                if (!messageDropdown.contains(e.target) && !messageMenu.contains(e.target)) {
                    messageMenu.classList.remove('dropdown-menu-show');
                }
            });
            ...
            function fetchMessages() {
                fetch('/messages/unread')
                    ...
                    .then(data => {
                        ...
                        messageMenu.innerHTML = ''; // Clear existing messages
                        
                        if (data.length > 0) {
                            data.forEach(message => {
                                const messageElement = document.createElement('a');
                                messageElement.href = `/messages/${message.id}`;
                                messageElement.className = 'dropdown-item';
                                messageElement.textContent = `${message.sender}: ${message.preview}`;
                                messageMenu.appendChild(messageElement);
                            });
                        } else {
                            const noMessagesElement = document.createElement('a');
                            noMessagesElement.href = '#';
                            noMessagesElement.className = 'dropdown-item';
                            noMessagesElement.textContent = 'No new messages';
                            messageMenu.appendChild(noMessagesElement);
                        }
                    })
            ...
            // New Message dropdown functionality
            const newMessageDropdown = document.getElementById('newMessageDropdown');
            const newMessageMenu = document.getElementById('newMessageMenu');
            const newMessageForm = document.getElementById('newMessageForm');

            newMessageDropdown.addEventListener('click', function(e) {
                e.preventDefault();
                newMessageMenu.classList.toggle('dropdown-menu-show');
            });

            // Close the new message dropdown when clicking outside
            document.addEventListener('click', function(e) {
                if (!newMessageDropdown.contains(e.target) && !newMessageMenu.contains(e.target)) {
                    newMessageMenu.classList.remove('dropdown-menu-show');
                }
            });
